...
from utils.functions import expands_loans_requests_reviews, upload_image
...
async def create_user(form: UserSchemaCreateForm = Depends(),
    ...
    """
    if profileImage:
        filepath = await upload_image(image=profileImage, folder="profiles")
    else:
...
async def get_users(db: AsyncSession = Depends(get_session),
    ...
    """
    async with db as session:
        users = await search_all_itens_in_db(Model=UserModel, db=db, filters=filters)
        try:
            return users
        except Exception as e:
            raise exception_not_identified(e)
...
async def put_user(user_id: int,
    ...
    async with db as session:
        ...
        if profileImage:
            user_db.profile_image = await upload_image(image=profileImage, folder="profiles")
        await session.commit()
