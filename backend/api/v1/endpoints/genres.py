...
from filters.genre_filter import GenreFilter
...
async def get_genres(db: AsyncSession = Depends(get_session),
    ...
    async with db as session:
        genres = await search_all_itens_in_db(db=session, Model=GenreModel, filters=filters)
        return genres
...
async def put_genre(genre_id: int,
    ...
    async with db as session:
        genre_db = await search_item_in_db(id=genre_id, db=session, Model=GenreModel)
        if not genre_db:
