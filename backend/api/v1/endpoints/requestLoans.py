...
from filters.request_filter import RequestsFilter
...
async def get_all(db: AsyncSession = Depends(get_session),
                  filters: RequestsFilter = Depends()
):
    async with db as session:
        requests = await search_all_itens_in_db(db=session, Model=LoanRequestModel, filters=filters)
        return requests
...
async def update(request_id: int, request: RequestLoanSchemaUpdate, db: AsyncSession = Depends(get_session), current_user: UserModel = Depends(get_current_user)):
    ...
    async with db as session:
        ...
        request_db.changer_by_id = current_user.id
        ...
        if request_db.status == StatusRequest.approved:
            ...
            try:
                ...
                await session.rollback()
                raise HTTPException(detail="Já existe um emprestimo ativo para esse usuário elivro.", status_code=status.HTTP_409_CONFLICT)
