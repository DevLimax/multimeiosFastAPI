...
from schemas.book_schema import BookSchemaBase, BookSchemaWithExtras, BookSchemaUpdateForm, BookSchemaForm
from filters.book_filter import BookFilter

from utils.search_in_db import *
from utils.exceptionsHttp import not_found, exception_not_identified, unauthorized
from utils.functions import upload_image, expands_loans_requests_reviews
...
async def get_books(db: AsyncSession = Depends(get_session),
    ...
    async with db as session:
        books = await search_all_itens_in_db(db=session, Model=BookModel, filters=filters)
        try:
            return books
...
#GET Book by ID
@router.get("/{book_id}", response_model=BookSchemaWithExtras, status_code=status.HTTP_200_OK)
...
async def create_book(form: BookSchemaForm = Depends(), 
    ...
    if fileCover:
        filepath = await upload_image(image=fileCover, folder="covers")
    else:
...
async def update_book(book_id: int,
    ...
    async with db as session:
        ...
        if fileCover:
            book_db.cover = await upload_image(image=fileCover, folder="covers")
